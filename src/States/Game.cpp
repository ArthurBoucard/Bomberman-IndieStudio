...
void Game::move_ai(std::size_t index)
{
    int rand = GetRandomValue(0, 4);

    printf("%d\n", rand);
    if (rand == 0)
        _positionList[index]->setX(_positionList[index]->getX() - _speed * 5);
    if (rand == 1)
        _positionList[index]->setX(_positionList[index]->getX() + _speed * 5);
    if (rand == 2)
        _positionList[index]->setY(_positionList[index]->getY() - _speed * 5);
    if (rand == 3)
        _positionList[index]->setY(_positionList[index]->getY() + _speed * 5);
    if (rand == 4)
    {
